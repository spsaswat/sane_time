...
public class Eating_Wkdy extends AppCompatActivity {
    String select;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_eating__wkdy);
        t1=(EditText)findViewById(R.id.editText56_e);
        t2=(EditText)findViewById(R.id.editText57_e);
        t3=(EditText)findViewById(R.id.editText58_e);
        t4=(EditText)findViewById(R.id.editText59_e);
        t5=(EditText)findViewById(R.id.editText60_e);
        t1_m=(EditText)findViewById(R.id.editText56_1_e);
        t2_m=(EditText)findViewById(R.id.editText57_1_e);
        t3_m=(EditText)findViewById(R.id.editText58_1_e);
        t4_m=(EditText)findViewById(R.id.editText59_1_e);
        t5_m=(EditText)findViewById(R.id.editText60_1_e);
    ...
    public void insertEAT(View view){
        UID=user.uid;
        ...
        s5= (!(t5_m.getText().toString().isEmpty()))?(t5.getText().toString()+":"+t5_m.getText().toString()):t5.getText().toString();
         ...
         if(t1_int<24 && t1_int>=0 && t2_int<24 && t2_int>=0 && t3_int<24 && t3_int>=0 && t4_int<24 && t4_int>=0 && t5_int<24 && t5_int>=0 && t1_int_m<60 && t1_int_m>=0 && t2_int_m<60 && t2_int_m>=0 && t3_int_m<60 && t3_int_m>=0 && t4_int_m<60 && t4_int>=0 && t5_int_m<60 && t5_int_m>=0) {
            boolean isin = myDb.insertDataEATING_WKDY(UID.getText().toString(), select, s1, s2, s3, s4, s5);
            if (isin == true)
                showMessage("Congrats!", "Data inserted!");
            else
                showMessage("Error!", "Could not be inserted! Check if you are trying to have two data for same day");
         }
         else
            showMessage("OOPs! Error","Data could not be inserted!");

    }
    public void view_allEAT(View view){
        UID=user.uid;
        Cursor res=myDb.get_all_dataEATING_WKDY(UID.getText().toString());
        if(res.getCount()==0){
    ...
    public void update_dataEAT(View view){
        UID=user.uid;
        String s1,s2,s3,s4,s5;
        s1= (!(t1_m.getText().toString().isEmpty()))?(t1.getText().toString()+":"+t1_m.getText().toString()):t1.getText().toString();
        s2= (!(t2_m.getText().toString().isEmpty()))?(t2.getText().toString()+":"+t2_m.getText().toString()):t2.getText().toString();
        ...
        if(t1_int<24 && t1_int>=0 && t2_int<24 && t2_int>=0 && t3_int<24 && t3_int>=0 && t4_int<24 && t4_int>=0 && t5_int<24 && t5_int>=0 && t1_int_m<60 && t1_int_m>=0 && t2_int_m<60 && t2_int_m>=0 && t3_int_m<60 && t3_int_m>=0 && t4_int_m<60 && t4_int>=0 && t5_int_m<60 && t5_int_m>=0) {
            boolean isUpdate = myDb.update_dbmsEATING_WKDY(UID.getText().toString(), select, s1, s2, s3, s4, s5);
            if (isUpdate == true)
    ...
    public void delete_dataEAT(View view){
        UID=user.uid;
        int r=myDb.delete_dbmsEATING_WKDY(UID.getText().toString(),select);
        if(r==1)
